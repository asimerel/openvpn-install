...
function installUnbound() {
	...
	if [[ ! -e /etc/unbound/unbound.conf ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			apt-get -o DPkg::Lock::Timeout=120 install -y unbound
...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			apt-get -o DPkg::Lock::Timeout=120 -y install ca-certificates gnupg
			...
			apt-get -o DPkg::Lock::Timeout=120 install -y openvpn iptables openssl wget ca-certificates curl
		elif [[ $OS == 'centos' ]]; then
...
function removeUnbound() {
	...
	if [[ $REMOVE_UNBOUND == 'y' ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			apt-get -o DPkg::Lock::Timeout=120 remove --purge -y unbound
		elif [[ $OS == 'arch' ]]; then
...
function removeOpenVPN() {
	...
	if [[ $REMOVE == 'y' ]]; then
		...
		if [[ $OS =~ (debian|ubuntu) ]]; then
			apt-get -o DPkg::Lock::Timeout=120 remove --purge -y openvpn
